...
final class StudioColorHarmonyTest extends TestCase {
	...
	public function test_light_first_layer_is_white_neutral_and_canvas_safe(): void {
		...
		self::assertStringContainsString( '--cc-color-surface: #ffffff', $rules );
		self::assertStringContainsString( '--cc-color-canvas: #f4f6f8', $rules );
		self::assertStringContainsString( '--cc-color-accent: #5b5cf6', $rules );
		self::assertStringContainsString( 'background-color: #eef1f5', $rules );
		self::assertStringContainsString( '.cc-studio-rail button.is-active', $rules );
		...
		self::assertStringNotContainsString( '.cc-studio-app button', $rules );
		self::assertStringNotContainsString( '.cc-studio-app input', $rules );
		self::assertStringNotContainsString( '.cc-studio-app select', $rules );
		self::assertStringNotContainsString( '.cc-studio-app textarea', $rules );
	}
	...
	public function test_theme_layers_do_not_style_rendered_document_roots(): void {
		...
		$combined = (string) preg_replace( '#/\*.*?\*/#s', '', $css . $light_css );
		$light_rules = (string) preg_replace( '#/\*.*?\*/#s', '', $light_css );
		...
		self::assertStringNotContainsString( '.cc-studio-canvas ', $light_rules );
		self::assertStringNotContainsString( '.cc-studio-canvas>', $light_rules );
	}
}
