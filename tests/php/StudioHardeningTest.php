...
final class StudioHardeningTest extends TestCase {
	...
	public function test_session_write_requires_the_checksum_loaded_by_the_editor(): void {
		$post_id = 42;
		$GLOBALS['cresco_test_posts'][ $post_id ] = (object) array(
			'ID' => $post_id,
			'post_type' => 'page',
			'post_title' => 'Hardening',
		);
		...
		$missing = new WP_REST_Request(
			array( 'postId' => $post_id, 'session' => $session ),
			'/cresco-canvas/v1/website-builder/session/' . $post_id,
			'POST'
		);
		...
		$stale = new WP_REST_Request(
			array( 'postId' => $post_id, 'session' => $session, 'baseChecksum' => str_repeat( '0', 64 ) ),
			'/cresco-canvas/v1/website-builder/session/' . $post_id,
			'POST'
		);
		...
		$current = new WP_REST_Request(
			array( 'postId' => $post_id, 'session' => $session, 'baseChecksum' => $checksum ),
			'/cresco-canvas/v1/website-builder/session/' . $post_id,
			'POST'
		);
