...
var cfg=window.crescoDiagnosticConfig||{},started=Date.now(),events=[],requests={},lastError=null,open=!!cfg.debug,renderQueued=false;
...
function add(type,data){events.push({at:now(),type:type,data:data||null});if(events.length>200)events.shift();render()}
...
add('probe.loaded',{postId:cfg.postId});document.addEventListener('DOMContentLoaded',function(){ensureLauncher();add('dom.ready')},{once:true});if(document.readyState!=='loading'){ensureLauncher();render()}
