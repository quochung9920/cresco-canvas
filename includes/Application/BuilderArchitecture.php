...
final class BuilderArchitecture {
	...
	public function register_routes() {
		register_rest_route( 'cresco-canvas/v1', '/website-builder/architecture/(?P<postId>\d+)', array( 'methods' => WP_REST_Server::READABLE, 'callback' => array( $this, 'rest_architecture' ), 'permission_callback' => array( $this, 'can_edit_document' ) ) );
		register_rest_route( 'cresco-canvas/v1', '/website-builder/scoped-context/(?P<postId>\d+)', array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'rest_scoped_context' ), 'permission_callback' => array( $this, 'can_edit_document' ) ) );
		register_rest_route( 'cresco-canvas/v1', '/website-builder/commands/(?P<postId>\d+)/preview', array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'rest_preview_command' ), 'permission_callback' => array( $this, 'can_edit_document' ) ) );
		register_rest_route( 'cresco-canvas/v1', '/website-builder/render/(?P<postId>\d+)', array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'rest_render' ), 'permission_callback' => array( $this, 'can_edit_document' ) ) );
	}
	...
	public function rest_architecture( WP_REST_Request $request ) {
		...
		return new WP_REST_Response( array(
			'schema' => 'cresco-builder-architecture/v1', 'documentType' => $this->document_type( $post_id ),
			'contracts' => array( 'document' => 'contracts/document/v1.schema.json', 'scope' => 'contracts/scope/v1.schema.json', 'command' => 'contracts/command/v1.schema.json', 'aiContext' => 'contracts/ai-context/v2.schema.json', 'patch' => 'cresco-patch/v1', 'interchange' => 'cresco-interchange/v1' ),
			'ui' => UiRegistry::manifest(), 'modules' => ModuleRegistry::all(), 'documents' => $this->document_index(), 'widgetTypes' => WidgetRegistry::types(),
			'capabilities' => array( 'scopedAI' => true, 'commandBus' => true, 'unifiedRender' => true, 'interchange' => true, 'themeDocuments' => post_type_exists( ThemeBuilder::POST_TYPE ), 'woocommerce' => class_exists( '\\WooCommerce' ) || defined( 'WC_VERSION' ) || function_exists( 'WC' ) ),
		) );
	}

	public function rest_scoped_context( WP_REST_Request $request ) {
		$post_id = absint( $request['postId'] ); $payload = (array) $request->get_json_params();
		$session = $this->session_from_payload( $post_id, $payload ); if ( is_wp_error( $session ) ) return $session;
		$scope = sanitize_key( (string) ( $payload['scope'] ?? 'document' ) ); $target = isset( $payload['target'] ) && is_array( $payload['target'] ) ? $payload['target'] : array();
		$purpose = sanitize_key( (string) ( $payload['purpose'] ?? 'edit' ) ); $mode = sanitize_key( (string) ( $payload['mode'] ?? 'auto' ) );
		$context = ContextEngine::build( $post_id, $session, $scope, $target, $purpose, $mode, array( 'documentType' => $this->document_type( $post_id ), 'postTitle' => get_the_title( $post_id ) ) );
	...
	public function enqueue_editor() {
		...
		$script = CRESCO_CANVAS_PATH . 'build/website-builder-architecture.js'; $style = CRESCO_CANVAS_PATH . 'assets/css/website-builder-architecture.css';
		...
		wp_add_inline_script( self::SCRIPT_HANDLE, 'window.crescoBuilderArchitectureSettings=' . wp_json_encode( array( 'postId' => $post_id, 'documentType' => $this->document_type( $post_id ), 'architecturePath' => '/cresco-canvas/v1/website-builder/architecture/' . $post_id, 'scopedContextPath' => '/cresco-canvas/v1/website-builder/scoped-context/' . $post_id, 'commandPreviewPath' => '/cresco-canvas/v1/website-builder/commands/' . $post_id . '/preview', 'renderPath' => '/cresco-canvas/v1/website-builder/render/' . $post_id, 'version' => 'architecture-v1' ) ) . ';', 'before' );
	}

	private function document_type( $post_id ) { return ( new WordPressDocumentRepository() )->type( $post_id ); }
	private function stored_session( $post_id ) { $session = ( new WordPressDocumentRepository() )->load( $post_id ); return is_wp_error( $session ) ? null : $session; }

	private function document_index() {
		$items = array();
		$pages = get_posts( array( 'post_type' => 'page', 'post_status' => array( 'publish', 'draft', 'private' ), 'posts_per_page' => 100, 'orderby' => 'modified', 'order' => 'DESC', 'meta_key' => WebsiteBuilder::BUILDER_META, 'meta_value' => WebsiteBuilder::BUILDER_VERSION ) ); // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key,WordPress.DB.SlowDBQuery.slow_db_query_meta_value -- Small builder-owned document index.
		foreach ( $pages as $page ) $items[] = array( 'id' => (int) $page->ID, 'type' => 'page', 'group' => 'Pages', 'title' => get_the_title( $page ), 'status' => $page->post_status, 'editUrl' => add_query_arg( array( 'page' => VisualEditor::PAGE_SLUG, 'post' => (int) $page->ID ), admin_url( 'admin.php' ) ) );
		...
		foreach ( $templates as $template ) { $type = $this->document_type( $template->ID ); $items[] = array( 'id' => (int) $template->ID, 'type' => $type, 'group' => in_array( $type, array( 'header', 'footer' ), true ) ? 'Theme' : 'Templates', 'title' => get_the_title( $template ), 'status' => $template->post_status, 'editUrl' => add_query_arg( array( 'page' => ThemeSessionBridge::PAGE_SLUG, 'post' => (int) $template->ID ), admin_url( 'admin.php' ) ) ); }
		return $items;
	}

	private function session_from_payload( $post_id, $payload ) {
		if ( isset( $payload['currentSession'] ) && is_array( $payload['currentSession'] ) ) return WebsiteBuilder::sanitize_session( $payload['currentSession'] );
		$session = ( new WordPressDocumentRepository() )->load( $post_id ); if ( is_wp_error( $session ) ) return $session;
		return $session ?: WebsiteBuilder::empty_session( $post_id );
	}

	private function asset_version( $path ) { $hash = is_readable( $path ) ? hash_file( 'sha256', $path ) : false; return CRESCO_CANVAS_VERSION . ( is_string( $hash ) && '' !== $hash ? '-' . substr( $hash, 0, 12 ) : '' ); }
}
