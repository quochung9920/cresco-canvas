<?php
/**
 * Final 0.8 Dynamic Data completion layer.
 *
...
final class DynamicCompletion {
	...
	/** Register final 0.8 blocks and REST services. */
	public function register() {
		add_action( 'init', array( $this, 'register_blocks' ), 35 );
		add_action( 'rest_api_init', array( $this, 'register_routes' ) );
	}

	/** Register row-aware structured blocks. */
	...
	/** Register signed facet-count and editor diagnostics routes. */
	...
	/** Render an image value from the active Repeater/Flexible row. */
	public function render_row_image( $attributes ) {
		$value = $this->row_value( $attributes['path'] ?? '' );
		$image = $this->normalize_image( $value );
		if ( ! $image['url'] ) {
			$image['url'] = esc_url_raw( (string) ( $attributes['fallbackUrl'] ?? '' ) );
		}
		if ( ! $image['url'] ) {
			return '';
		}
		...
		$html      = $image['id']
			? wp_get_attachment_image( $image['id'], $size, false, array( 'alt' => $alt, 'loading' => 'lazy' ) )
			: '<img src="' . esc_url( $image['url'] ) . '" alt="' . esc_attr( $alt ) . '" loading="lazy" decoding="async" />';
		return $html ? '<figure ' . get_block_wrapper_attributes( array( 'class' => 'cresco-acf-row-image' ) ) . '>' . $html . '</figure>' : '';
	}

	/** Render a gallery value from the active row. */
	public function render_row_gallery( $attributes ) {
		...
		foreach ( array_slice( $images, 0, $limit ) as $image ) {
			$html = $image['id']
				? wp_get_attachment_image( $image['id'], $size, false, array( 'alt' => $image['alt'], 'loading' => 'lazy' ) )
				: '<img src="' . esc_url( $image['url'] ) . '" alt="' . esc_attr( $image['alt'] ) . '" loading="lazy" decoding="async" />';
			if ( $html ) {
				$items .= '<figure class="cresco-acf-row-gallery__item">' . $html . '</figure>';
			}
		}
	...
	/** Render related posts from the active row using native inner blocks. */
	...
	/** Return dependent facet counts for a signed Filterable Loop payload. */
	public function facet_counts( WP_REST_Request $request ) {
		...
		if ( ! InteractiveQuery::verify_payload( $encoded, $signature ) ) {
		...
		foreach ( array_slice( $facets, 0, 3 ) as $taxonomy ) {
			...
			foreach ( $terms as $term ) {
				...
				foreach ( $filters['tax'] as $filter_taxonomy => $slugs ) {
					if ( $filter_taxonomy !== $taxonomy && $slugs ) {
						$tax_query[] = array( 'taxonomy' => $filter_taxonomy, 'field' => 'slug', 'terms' => $slugs );
					}
				}
	...
	/** Scan one post for duplicate loop IDs and unsafe dynamic configurations. */
	public function diagnostics( WP_REST_Request $request ) {
		$post = get_post( absint( $request['id'] ) );
		if ( ! $post ) {
			return new WP_REST_Response( array( 'issues' => array() ), 404 );
		}
		$issues = array();
		$seen   = array();
	...
	/** Recursively scan native blocks. */
	private function scan_blocks( $blocks, &$seen, &$issues ) {
		foreach ( (array) $blocks as $block ) {
			...
			if ( 'cresco/filterable-loop' === $name ) {
				...
				if ( isset( $seen[ $id ] ) ) {
					$issues[] = array( 'code' => 'duplicate_instance_id', 'message' => 'Filterable Loops must use unique Instance IDs.', 'instanceId' => $id );
				}
				$seen[ $id ] = true;
				if ( empty( $attrs['facetTaxonomies'] ) && empty( $attrs['searchFilter'] ) ) {
					$issues[] = array( 'code' => 'no_filters', 'message' => 'Filterable Loop has no enabled filters.', 'instanceId' => $id );
				}
			}
			if ( in_array( $name, array( self::ROW_IMAGE, self::ROW_GALLERY, self::ROW_RELATIONSHIP ), true ) && empty( $attrs['path'] ) ) {
				$issues[] = array( 'code' => 'missing_row_path', 'message' => 'Row binding block is missing its field path.', 'block' => $name );
			}
			if ( ! empty( $block['innerBlocks'] ) ) {
				$this->scan_blocks( $block['innerBlocks'], $seen, $issues );
			}
		}
	}

	/** Read a safe path from the current structured row. */
	private function row_value( $path ) {
		$path = StructuredDynamicData::sanitize_path( $path );
		if ( ! $path || ! method_exists( StructuredDynamicData::class, 'current_row' ) ) {
			return null;
		}
		$row = StructuredDynamicData::current_row();
		return is_array( $row ) ? StructuredDynamicData::resolve_path( $row, $path ) : null;
	}

	/** Normalize one common image return value. */
