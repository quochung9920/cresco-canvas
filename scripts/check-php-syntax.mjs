...
if (probe.error || probe.status !== 0) {
  console.error(`PHP syntax check requires a working PHP CLI. Tried: ${phpBinary}`);
  if (probe.error) {
...
if (phpFiles.length === 0) {
  console.error('PHP syntax check found no PHP files. Refusing to pass an empty gate.');
  process.exit(1);
}
...
if (failures.length > 0) {
  ...
  for (const failure of failures) {
    console.error(`\n--- ${failure.file} ---`);
    console.error(failure.output || 'php -l failed without diagnostic output.');
  }
  process.exit(1);
}
