...
if ( ! existsSync( path.join( root, POT ) ) ) {
	errors.push(
		`Missing translation catalogue: ${ POT }. Run \`npm run make:pot\`.`
	);
} else {
	...
	for ( const header of [
		...
		if ( ! contents.includes( header ) ) {
			errors.push( `${ POT } is missing header: ${ header }` );
		}
	}

	const committed = ( contents.match( /^msgid "/gm ) || [] ).length - 1;
	if ( committed < 1 ) {
		errors.push( `${ POT } carries no translatable strings.` );
	}
	...
	try {
		...
		reported = Number(
			( output.match( /\[make-pot\] (\d+) unique/ ) || [] )[ 1 ] || 0
		);
	} catch ( error ) {
