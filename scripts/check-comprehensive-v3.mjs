...
const [interchange,componentSync,v3Php,plugin,source,build,css,manifestText,releaseFiles,pkgText]=await Promise.all([
 read('includes/Builder/WebsiteBuilderInterchange.php'),read('includes/Builder/WebsiteBuilderComponentSync.php'),read('includes/Builder/WebsiteBuilderComprehensiveV3.php'),read('includes/Plugin.php'),read('runtime-src/build/website-builder-comprehensive-v3.js'),read('build/website-builder-comprehensive-v3.js'),read('assets/css/website-builder-comprehensive-v3.css'),read('runtime-src/manifest.json'),read('scripts/release-files.mjs'),read('package.json')
]);
...
for(const t of ['WebsiteBuilderInterchange','WebsiteBuilderComponentSync','WebsiteBuilderComprehensiveV3','( new WebsiteBuilderInterchange() )->register();','( new WebsiteBuilderComponentSync() )->register();','( new WebsiteBuilderComprehensiveV3() )->register();'])if(!plugin.includes(t))errors.push(`Plugin registration missing ${t}`);
...
for(const t of ["'assets/css/website-builder-comprehensive-v3.css'","'build/website-builder-comprehensive-v3.js'"])if(!releaseFiles.includes(t))errors.push(`Release allowlist missing ${t}`);
...
if(errors.length){process.stderr.write(errors.join('\n')+'\n');process.exit(1);}process.stdout.write('Comprehensive Builder V3 interchange, parity, component sync, accessibility/performance diagnostics, source/build parity and release inventory verified.\n');
